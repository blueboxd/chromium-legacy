...
WaitableEvent::WaitableEvent(ResetPolicy reset_policy,
  ...
  kr = mach_port_insert_right(mach_task_self(), name, name,
...
size_t WaitableEvent::WaitMany(WaitableEvent** raw_waitables, size_t count) {
  ...
  const WaitManyPrimitive kPrimitive =
      mac::IsAtLeastOS10_12() ? KQUEUE : DISPATCH;
#endif
...
bool WaitableEvent::PeekPort(mach_port_t port, bool dequeue) {
  ...
  if (kr == KERN_SUCCESS) {
    if (!dequeue) {
      ...
      // If the event is already signaled, this will time out because the queue
      // has a length of one.
      kr = mach_msg(&sendmsg.header, MACH_SEND_MSG | MACH_SEND_TIMEOUT,
                    sizeof(sendmsg), 0, MACH_PORT_NULL, 0, MACH_PORT_NULL);
